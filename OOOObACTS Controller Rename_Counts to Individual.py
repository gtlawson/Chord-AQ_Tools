...
This script uses raw pneumatic controller data from ACTS, which is structured
as a count of makes and models by facility, to create simple individual 
controller names by creating a sequential number for each one.  

To use, update File PAth section with location of data, and verify columns are
the same.  The source file is an ACTS report that includes the pneumatic
controller data plus a column for OOOOb facility flag.
"""
